...
function Sentiment({ label, type }: { label: string; type: string }) {
  const c = type === 'bullish' ? 'bg-green-500/10 text-green-400 border-green-500/20' : type === 'bearish' ? 'bg-red-500/10 text-red-400 border-red-500/20' : 'bg-gray-500/10 text-gray-400 border-gray-500/20';
  return <span className={`px-2 py-0.5 rounded text-[10px] border ${c}`}>{label}</span>;
}
  ...
  const load = useCallback(async () => {
    try {
      const [i, s, w] = await Promise.all([
        fetch('/api/index?sort=trending').then(r => r.json()),
        fetch('/api/smart-money').then(r => r.json()),
        fetch('/api/whale-tracker').then(r => r.json()),
      ]);
      // All 3 endpoints return the same data structure now (DexScreener based)
      // but we combine them into one feed
      const allTokens = i.tokens || [];
      setTokens(allTokens);
    ...
    <div>
      ...
      {loading && tokens.length === 0 ? (
        ...
        <div className="bg-white/[0.03] border border-white/10 rounded-2xl overflow-hidden">
          {/* Header */}
          <div className="hidden md:grid md:grid-cols-[1fr_100px_100px_100px_80px_80px_100px_80px] gap-2 px-4 py-3 border-b border-white/10 text-[11px] text-gray-500 font-medium uppercase tracking-wider">
          ...
          <div className="divide-y divide-white/5">
            {tokens.map((t, i) => (
              <a key={`${t.contractAddress}-${i}`} href={`/token/${t.contractAddress}`}
                className="grid grid-cols-[1fr_90px_90px_90px_70px_60px_90px] md:grid-cols-[1fr_100px_100px_100px_80px_80px_100px_80px] gap-2 px-4 py-3 hover:bg-white/[0.03] transition items-center group">
                ...
                <Sentiment label={t.sentiment} type={t.sentimentType} />
      ...
      <p className="text-center text-[11px] text-gray-600 mt-6">
        Data from DexScreener • Auto-refreshes 30s • Built by Ragna
      </p>
    </div>
  );
}
